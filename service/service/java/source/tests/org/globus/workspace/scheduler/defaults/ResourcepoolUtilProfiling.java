...
public class ResourcepoolUtilProfiling extends NimbusDatabaseTestCase {
    ...
    public static void init() throws Exception {
        ...
        for (int i = 0; i < NUM_ENTRIES; i++) {
            ...
            entries1.put(hostname, new ResourcepoolEntry("testrp1", hostname, Math.max(mem1, mem2), Math.min(mem1, mem2), ""));
            entries2.put(hostname, new ResourcepoolEntry("testrp1", hostname, Math.max(mem1, mem2), Math.min(mem1, mem2), "")); 
            entries3.put(hostname, new ResourcepoolEntry("testrp1", hostname, Math.max(mem1, mem2), Math.min(mem1, mem2), ""));
        }
    ...
    public double profileGetResourcePoolEntryNormal(int requestedMem, int executions) throws Exception {    
        ...
        for(int i=0; i<executions; i++){
            ...
            String hostname = ResourcepoolUtil.getResourcepoolEntry(requestedMem, new String[0], adapter, lagerImpl, 1, true);
            end = System.nanoTime();
    ...
    public double profileGetResourcePoolEntryImproved(int requestedMem, int executions) throws Exception {    
        ...
        for(int i=0; i<executions; i++){
            init = System.nanoTime();
            String hostname = ResourcepoolUtil.getResourcePoolEntryImproved(requestedMem, new String[0], adapter, lagerImpl, 1, true);
            end = System.nanoTime();
